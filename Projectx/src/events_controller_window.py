...
# Сохранять рядом с этим модулем, чтобы путь был стабилен при перезапусках
SETTINGS_FILE = os.path.join(os.path.dirname(__file__), "events_settings.json")
...
class EventsControllerWindow(tk.Toplevel):
    def __init__(self, master, hp_action_controller):
        ...
        tk.Label(
            self,
            text="Команда при отсутствии цели (для последовательности разделяйте ';'): ",
        ...
        tk.Label(
            self,
            text="Команда при мёртвой цели (для последовательности разделяйте ';'): ",
        ).pack(anchor="w", padx=10, pady=(10, 0))
        ...
        tk.Label(
            self,
            text="Команда при живой цели (для последовательности разделяйте ';'): ",
        ).pack(anchor="w", padx=10, pady=(10, 0))
        ...
        tk.Label(
            self,
            text="Команда при далёкой цели (far_target) (для последовательности разделяйте ';'): ",
        ).pack(anchor="w", padx=10, pady=(10, 0))
        ...
        tk.Checkbutton(
            self,
            text="far — одноразовое действие (transient)",
            variable=self.far_transient_var,
        ).pack(anchor="w", padx=10, pady=(10, 0))
    ...
    def load_settings(self):
        # Для отладки выводим, где ищем файл
        print(
            f"[EventsController] Loading settings from: {SETTINGS_FILE} (cwd: {os.getcwd()})"
        )
        if os.path.exists(SETTINGS_FILE):
            try:
                ...
                self.no_target_var.set(
                    self._parse_sequence_field(data.get("no_target_command", ""))
                )
                self.dead_target_var.set(
                    self._parse_sequence_field(data.get("dead_target_command", ""))
                )
                self.alive_target_var.set(
                    self._parse_sequence_field(data.get("alive_target_command", ""))
                )
                ...
                self.far_target_var.set(
                    self._parse_sequence_field(data.get("far_target_command", ""))
                )
                ...
                self.hp_action_controller.set_no_target_command(
                    self._to_list(self.no_target_var.get())
                )
                self.hp_action_controller.set_dead_target_command(
                    self._to_list(self.dead_target_var.get())
                )
                self.hp_action_controller.set_alive_target_command(
                    self._to_list(self.alive_target_var.get())
                )
                ...
                self.hp_action_controller.set_far_target_command(
                    self._to_list(self.far_target_var.get())
                )
                ...
                self.hp_action_controller.set_far_transient(
                    self.far_transient_var.get()
                )

                self.status_label.config(text="Настройки загружены из файла")
            except Exception as e:
                # показать реальное исключение
                self.status_label.config(text=f"Ошибка загрузки: {e}")
                self.update_fields_from_controller()
        else:
            self.status_label.config(text=f"Файл настроек не найден: {SETTINGS_FILE}")
            self.update_fields_from_controller()

    def update_fields_from_controller(self):
        self.no_target_var.set(
            ";".join(getattr(self.hp_action_controller, "no_target_sequence", []))
        )
        self.dead_target_var.set(
            ";".join(getattr(self.hp_action_controller, "dead_target_sequence", []))
        )
        self.alive_target_var.set(
            ";".join(getattr(self.hp_action_controller, "alive_target_sequence", []))
        )
        ...
        self.far_target_var.set(
            ";".join(getattr(self.hp_action_controller, "far_target_sequence", []))
        )
        ...
        self.far_transient_var.set(
            getattr(self.hp_action_controller, "far_transient", True)
        )
    ...
    def save_settings(self):
        ...
        try:
            print(
                f"[EventsController] Saving settings to: {SETTINGS_FILE} (cwd: {os.getcwd()})"
            )
            with open(SETTINGS_FILE, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            self.status_label.config(
                text=f"Настройки сохранены (и применены) — файл: {SETTINGS_FILE}"
            )
        except Exception as e:
